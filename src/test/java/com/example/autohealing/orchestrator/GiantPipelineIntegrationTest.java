...
public class GiantPipelineIntegrationTest {
        ...
        void testEndToEndGiantPipelineFlow() throws Exception {
                ...
                java.util.Map<String, Object> rawVuln = java.util.Map.of(
                                ...
                                "version", "1.0.0");
                when(snykClient.scan(anyString())).thenReturn(java.util.List.of(rawVuln));

                // [Given] 2. AI 수정 서비스가 패치 코드를 성공적으로 생성했다고 가정
                AiRemediationService mockAiService = org.mockito.Mockito.mock(AiRemediationService.class);
                when(aiManager.getActiveAi()).thenReturn(mockAiService);
                ...
                when(mockAiService.fixCode(anyString(), anyString())).thenReturn(mockAiResult);
                when(mockAiService.providerName()).thenReturn("Gemini-Prod");
                ...
                orchestrator.runScanAndUpdate(parentIssueKey, "my-repo");

                // [Then] 전체 파이프라인 단계별 호출 검증 (비동기 처리를 위해 timeout 속성 적용)
                // 1. Snyk 스캔 호출 확인
                verify(snykClient, timeout(5000)).scan(anyString());

                // 2. AI 수정 서비스 호출 확인 (Critical 등급이므로 호출되어야 함)
                verify(mockAiService, timeout(20000)).fixCode(anyString(), contains("SNYK-JAVA-SPRING-12345"));

                // 3. 컴파일 검증 호출 확인
                verify(codeValidatorService, timeout(20000)).validateCode(eq(mockAiResult.getFixedCode()), anyString());

                // 4. Jira 개별 취약점 티켓 생성 및 진행 상태 변경 확인
                verify(jiraService, timeout(20000)).createIssue(contains("Insecure Deserialization in Spring"),
                                anyString(),
                                eq("CRITICAL"), anyList());
                ...
                verify(discordNotificationService, timeout(20000).atLeastOnce()).sendSnykAlert(anyString(), anyString(),
                                anyString());
                verify(discordNotificationService, timeout(20000)).sendPrCreatedAlert(anyString(), contains("777"),
                                anyString());

                // 7. DB 최종 결과 업데이트 루틴 확인
                verify(securityLogRepository, timeout(20000).atLeast(2)).save(any(SecurityLog.class));
        }
}
